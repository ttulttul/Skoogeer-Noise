...
| [Mustache Variable Sampler](#mustache-variable-sampler) | `text/template` | `MUSTACHE_VARIABLES`, `INT` |
...
Passes a `MUSTACHE_VARIABLES` mapping through in either original or randomized per-variable order and emits a companion `limit` integer for downstream `Mustache Template` nodes.

- **Menu category:** `text/template`
- **Returns:** `MUSTACHE_VARIABLES`, `INT`
...
| `limit` | `INT` | `-1` | `-1..2147483647` | Maximum number of rendered prompts to emit. `-1` disables the cap and allows the full Cartesian product. This can be driven directly from `Mustache Variable Sampler.limit`. |

##### How permutation expansion works

The node first scans the template for placeholders in first-appearance order. It then looks up each referenced variable's candidate values and streams the resulting Cartesian product in order, stopping once `limit` outputs have been produced (unless `limit = -1`).
