...
Passes a `MUSTACHE_VARIABLES` mapping through in either original or randomized per-variable order.
...
- This node does not expand templates itself; it only prepares variable ordering.
...
| `limit` | `INT` | `-1` | `-1..2147483647` | Maximum number of rendered prompts to emit. `-1` disables the cap and allows the full Cartesian product. |
