...
For a larger tracked YAML sample that follows that guide's layered approach and demonstrates weighted optional clauses, clause reuse, deeper nested composition, and semantically tuned sampled outputs, see [`sample.yaml`](sample.yaml).
...
- The `markdown` socket is emitted as a single list-aware `STRING` item so downstream text nodes receive the Markdown document itself rather than a Python list wrapper.
