...
Renders a template string containing mustache placeholders like `{{haircolor}}` against a `MUSTACHE_VARIABLES` mapping and outputs every permutation as a list of strings. Because the output is a ComfyUI `STRING` list, downstream nodes such as `CLIP Text Encode` will execute once per rendered prompt.
...
| Field | Type | Default | Range/Options | Notes |
|------|------|---------|--------------|------|
| `variables` | `MUSTACHE_VARIABLES` | – | – | Variable mapping generated by `Mustache Variables`. Each key maps to one or more candidate text values. |
| `template` | `STRING` | `"The man has {{haircolor}} hair and {{leglength}} legs."` | multiline | Template text to render. Every referenced placeholder must exist in `variables`. Repeated placeholders reuse the same selected value within a given permutation. |
| `limit` | `INT` | `0` | `0..2147483647` | Maximum number of rendered prompts to emit. `0` disables the cap and allows the full Cartesian product. |
| `sampling_method` | enum | `sequential` | `sequential/random` | How permutations are selected when rendering. `sequential` walks the Cartesian product in normal order. `random` samples unique permutations in randomized order. |

##### How permutation expansion works

The node first scans the template for placeholders in first-appearance order. It then looks up each referenced variable's candidate values and treats the result as a Cartesian product.

- With `sampling_method = sequential`, the node streams permutations in normal Cartesian-product order and stops as soon as `limit` outputs have been produced.
- With `sampling_method = random`, the node samples unique permutation indices from the product space and decodes only those combinations. When `limit > 0`, it does not first materialize the full permutation list just to shuffle and truncate it.
...
- Variables present in `MUSTACHE_VARIABLES` but not referenced in the template are ignored.
...
- `random` mode is intentionally non-deterministic because this node currently does not expose a seed input.
